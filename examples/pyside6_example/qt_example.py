...
def get_interfaces():
    result = QNetworkInterface.allAddresses()

    for line in result:
        if QAbstractSocket.NetworkLayerProtocol.IPv4Protocol  == line.protocol():
            print(f"Ip: {line.toString()}")
            yield line.toString()


class QDiscovery(QObject):
    discoveringChanged = Signal(bool)
    discoveredQTM = Signal(str, str)
    ...
    @Property(bool, notify=discoveringChanged)
